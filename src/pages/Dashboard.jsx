...
import { FileText, TrendingUp, Clock, AlertCircle, Users, Receipt, ChevronRight, BarChart2 } from 'lucide-react'
import {
...
export default function Dashboard() {
  ...
  // ── Stats filtered to current year ──────────────────────────────────────
  const stats = useMemo(() => {
    const yearInv     = invoices.filter(i => new Date(i.date).getFullYear() === THIS_YEAR)
    const yearExp     = expenses.filter(e => new Date(e.date).getFullYear() === THIS_YEAR)

    const paidInvoices    = yearInv.filter(i => i.status === 'paid')
    const pendingInvoices = yearInv.filter(i => i.status === 'pending')
    const overdueInvoices = invoices.filter(i => i.status === 'pending' && new Date(i.dueDate) < new Date())

    const totalBilled   = yearInv.filter(i => i.status !== 'draft').reduce((s, i) => s + calcDocumentTotals(i.lines).total, 0)
    ...
    const totalExpenses = yearExp.reduce((s, e) => s + e.amount * (1 + e.tax / 100), 0)
  ...
  const monthlyData = useMemo(() => {
    ...
    invoices
      .filter(inv => new Date(inv.date).getFullYear() === THIS_YEAR)
      .forEach(inv => {
  ...
  const pieData = useMemo(() => {
    const groups = {}
    invoices
      .filter(i => new Date(i.date).getFullYear() === THIS_YEAR)
      .forEach(inv => {
    ...
    <div className="space-y-5 animate-slide-in">
      ...
      <div className="pt-1 pb-2">
        ...
        <p className="text-[14px] text-gray-400 dark:text-[#636366] mt-0.5">
          ...
          {' · '}
          <span className="font-medium text-[#007AFF]">Año {THIS_YEAR}</span>
        </p>
      </div>

      {/* ── Stat cards (año actual) ── */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        <StatCard title="Facturado" value={formatCurrency(stats.totalBilled)}
          subtitle={`Este año · ${invoices.filter(i => i.status !== 'draft' && new Date(i.date).getFullYear() === THIS_YEAR).length} facturas`}
          icon={TrendingUp} color="primary" />
        <StatCard title="Cobrado" value={formatCurrency(stats.totalPaid)}
          subtitle={`${invoices.filter(i => i.status === 'paid' && new Date(i.date).getFullYear() === THIS_YEAR).length} cobradas`}
          icon={FileText} color="green" />
        <StatCard title="Pendiente" value={formatCurrency(stats.totalPending)}
          subtitle={`${stats.pendingInvoices.length} pendientes`}
          icon={Clock} color="yellow" />
        <StatCard title="Gastos" value={formatCurrency(stats.totalExpenses)}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#1C1C1E] rounded-xl2 shadow-card p-5">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white tracking-tight">Facturación mensual</h3>
              <p className="text-[12px] text-gray-400 dark:text-[#636366] mt-0.5">{THIS_YEAR}</p>
            </div>
            <div className="flex items-center gap-3 text-[11px] text-gray-400 dark:text-[#636366]">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#007AFF]" />Facturado</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#34C759]" />Cobrado</span>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={monthlyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="facturado" stroke="#007AFF" fill="url(#gF)" strokeWidth={2} dot={false}
                activeDot={{ r: 4, fill: '#007AFF', strokeWidth: 0 }} name="Facturado" />
              <Area type="monotone" dataKey="cobrado" stroke="#34C759" fill="url(#gC)" strokeWidth={2} dot={false}
                activeDot={{ r: 4, fill: '#34C759', strokeWidth: 0 }} name="Cobrado" />
            </AreaChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="bg-white dark:bg-[#1C1C1E] rounded-xl2 shadow-card p-5">
          <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white tracking-tight mb-1">Estado</h3>
          <p className="text-[12px] text-gray-400 dark:text-[#636366] mb-3">Facturas {THIS_YEAR}</p>
          {pieData.length === 0 ? (
            <div className="flex items-center justify-center h-[190px] text-[13px] text-gray-400 dark:text-[#636366]">Sin facturas este año</div>
          ) : (
            <ResponsiveContainer width="100%" height={190}>
              <PieChart>
                <Pie data={pieData} cx="50%" cy="42%" innerRadius={52} outerRadius={76} paddingAngle={2} dataKey="value" strokeWidth={0}>
                  {pieData.map((entry, i) => <Cell key={entry.key} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                </Pie>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-[#1C1C1E] rounded-xl2 shadow-card">
          <div className="flex items-center justify-between px-5 pt-5 pb-3">
            <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white tracking-tight">Facturas recientes</h3>
            <Link to="/app/facturas" className="flex items-center gap-0.5 text-[13px] text-[#007AFF] font-medium hover:opacity-70 transition-opacity">
              Ver todas <ChevronRight size={14} />
            </Link>
          </div>
          <div className="divide-y divide-black/[0.04] dark:divide-white/[0.04]">
            {recentInvoices.length === 0 ? (
              <p className="px-5 py-6 text-[13px] text-gray-400 dark:text-[#636366]">No hay facturas aún</p>
            ) : recentInvoices.map(inv => {
              const client = getClient(inv.clientId)
              const total  = calcDocumentTotals(inv.lines).total
              return (
                <Link key={inv.id} to="/app/facturas"
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-[13px] font-medium text-gray-900 dark:text-white truncate leading-snug">{client?.name || 'Cliente desconocido'}</p>
                    <p className="text-[11px] text-gray-400 dark:text-[#636366] mt-0.5">{inv.number} · {formatDate(inv.date)}</p>
                  </div>
                  <div className="flex items-center gap-3 ml-4 flex-shrink-0">
                    <Badge status={inv.status} />
                    <span className="text-[13px] font-semibold text-gray-900 dark:text-white tabular-nums">{formatCurrency(total)}</span>
        ...
        {/* Alerts */}
        <div className="bg-white dark:bg-[#1C1C1E] rounded-xl2 shadow-card p-5">
          <h3 className="text-[15px] font-semibold text-gray-900 dark:text-white tracking-tight mb-4">Alertas</h3>
          <div className="space-y-2.5">
            {stats.overdueInvoices.length > 0 && (
              <Link to="/app/facturas"
            ...
            <div className="flex items-center gap-3.5 p-3.5 bg-[#007AFF]/[0.06] dark:bg-[#007AFF]/[0.08] rounded-xl">
              ...
              <div>
                <p className="text-[13px] font-semibold text-[#0062cc] dark:text-[#409cff] leading-snug">{clients.length} clientes activos</p>
                <p className="text-[11px] text-[#007AFF]/70 mt-0.5">En tu cartera</p>
              </div>
            </div>
            {/* Analytics shortcut */}
            <Link to="/app/analitica"
              className="flex items-center gap-3.5 p-3.5 bg-[#AF52DE]/[0.06] dark:bg-[#AF52DE]/[0.08] rounded-xl hover:bg-[#AF52DE]/[0.12] dark:hover:bg-[#AF52DE]/[0.14] transition-colors group">
              <div className="w-8 h-8 rounded-[10px] bg-[#AF52DE] flex items-center justify-center flex-shrink-0 shadow-sm">
                <BarChart2 size={15} className="text-white" />
              </div>
              <div className="flex-1">
                <p className="text-[13px] font-semibold text-[#8944ab] dark:text-[#bf5af2] leading-snug">Ver análisis completo</p>
                <p className="text-[11px] text-[#AF52DE]/70 mt-0.5">Clientes, comparativas y más</p>
              </div>
              <ChevronRight size={14} className="text-[#AF52DE]/50 group-hover:text-[#AF52DE] transition-colors" />
            </Link>
            {stats.overdueInvoices.length === 0 && quotes.filter(q => q.status === 'pending').length === 0 && (
              <p className="text-[13px] text-gray-400 dark:text-[#636366] text-center py-2">✓ Todo al día. ¡Buen trabajo!</p>
            )}
          </div>
        </div>
      </div>
