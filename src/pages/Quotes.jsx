...
export default function Quotes() {
  ...
  function save(data) {
    if (selected) {
      dispatch({ type: 'UPDATE_QUOTE', payload: { ...selected, ...data } })
    } else {
      dispatch({ type: 'ADD_QUOTE', payload: { ...data, id: generateId('q') } })
    }
    setModal(null)
  }
  ...
  function convertToInvoice(q) {
    ...
    const invoiceNumber = `FAC-${new Date().getFullYear()}-${String(state.invoices.length + 1).padStart(3, '0')}`
    ...
    setModal(null)
    alert(`Presupuesto convertido a factura ${invoiceNumber}`)
  }

  const columns = [
    {
      header: 'Número',
      render: q => <span className="font-mono text-sm font-medium text-gray-900">{q.number}</span>,
    },
    {
      header: 'Cliente',
      render: q => <span className="text-gray-700">{getClient(q.clientId)?.name || '-'}</span>,
    },
    {
      header: 'Fecha',
      render: q => <span className="text-gray-500 text-sm">{formatDate(q.date)}</span>,
    },
    {
      header: 'Válido hasta',
      render: q => {
    ...
    },
    {
      ...
      render: q => (
        <span className="font-semibold text-gray-900">{formatCurrency(calcDocumentTotals(q.lines).total)}</span>
      ),
    },
    {
      header: '',
      ...
      render: q => (
        <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
          <button onClick={() => openView(q)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-primary-600">
            <Eye size={14} />
          </button>
          <button onClick={() => openEdit(q)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600">
            <Pencil size={14} />
          </button>
          <button onClick={() => remove(q.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500">
            <Trash2 size={14} />
          </button>
        </div>
      ),
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal
        ...
        title={selected ? `Editar ${selected.number}` : 'Nuevo presupuesto'}
        size="xl"
      >
        <InvoiceForm
          initial={selected}
      ...
      <Modal
        open={modal === 'view'}
        onClose={() => setModal(null)}
        title={selected?.number || 'Presupuesto'}
        size="lg"
      >
        {selected && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 flex-wrap pb-4 border-b border-gray-100">
              ...
              <button
                ...
                className="ml-auto flex items-center gap-1.5 px-3 py-1.5 text-xs bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 text-green-700 font-medium"
              >
