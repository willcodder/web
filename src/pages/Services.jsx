...
export default function Services() {
  ...
  function openEdit(p) { setSelected(p); setForm({ ...p }); setModal('edit') }
  ...
  function remove(id) {
    if (confirm('¿Eliminar este servicio?')) dispatch({ type: 'DELETE_PRODUCT', payload: id })
  }
  ...
  const columns = [
    {
      ...
      render: p => (
        <div>
          ...
          {p.description && <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5 truncate max-w-xs">{p.description}</p>}
        </div>
      ),
    },
    {
      header: 'Categoría',
      render: p => <span className="inline-flex px-2 py-0.5 rounded-full text-xs font-medium bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400">{p.category}</span>,
    },
    { header: 'Unidad', render: p => <span className="text-gray-600 dark:text-gray-300 capitalize">{p.unit}</span> },
    { header: 'Precio', cellClassName: 'text-right', render: p => <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(p.price)}</span> },
    { header: 'IVA', cellClassName: 'text-center', render: p => <span className="text-gray-600 dark:text-gray-300">{p.tax}%</span> },
    {
      header: '', cellClassName: 'text-right',
      render: p => (
        <div className="flex items-center justify-end gap-2" onClick={e => e.stopPropagation()}>
          <button onClick={() => openEdit(p)} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200"><Pencil size={14} /></button>
          <button onClick={() => remove(p.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-400 hover:text-red-500"><Trash2 size={14} /></button>
        </div>
      ),
    },
  ]
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p className="text-sm text-gray-500 dark:text-gray-400">{state.products.length} servicios en catálogo</p>
        ...
        {categories.map(cat => {
          ...
          return (
            <button key={cat} onClick={() => setFilterCat(filterCat === cat ? '' : cat)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${filterCat === cat ? 'bg-primary-600 text-white border-primary-600' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 hover:border-primary-300'}`}>
              <Package size={12} />{cat} <span className="opacity-70">({count})</span>
            </button>
          )
        })}
      </div>
      ...
      <Modal open={modal === 'new' || modal === 'edit'} onClose={() => setModal(null)} title={modal === 'new' ? 'Nuevo servicio' : 'Editar servicio'}>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nombre del servicio *</label>
            <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} className={inputCls} placeholder="Ej: Día de rodaje completo" />
          </div>
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descripción</label>
            <input type="text" value={form.description || ''} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} className={inputCls} placeholder="Descripción breve" />
          </div>
          <div>
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={save} className="px-4 py-2 text-sm bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium">{modal === 'new' ? 'Crear servicio' : 'Guardar cambios'}</button>
        </div>
      </Modal>
    </div>
  )
