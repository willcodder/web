...
export default function Expenses() {
  ...
  const totalAll = totalBase + totalTax
  ...
  const columns = [
    {
      ...
      render: e => (
        <div>
          ...
          <p className="text-xs text-gray-400">{e.provider}</p>
        </div>
      ),
    },
    {
    ...
    {
      header: 'Fecha',
      render: e => <span className="text-gray-500 text-sm">{formatDate(e.date)}</span>,
    },
    {
      header: 'Base',
      cellClassName: 'text-right',
      render: e => <span className="text-gray-700">{formatCurrency(e.amount)}</span>,
    },
    {
      header: 'IVA',
      cellClassName: 'text-center',
      render: e => <span className="text-gray-500 text-sm">{e.tax}%</span>,
    },
    {
      header: 'Total',
      cellClassName: 'text-right',
      render: e => <span className="font-semibold text-gray-900">{formatCurrency(e.amount * (1 + e.tax / 100))}</span>,
    },
    {
      header: 'Estado',
    ...
    },
    {
      ...
      render: e => (
        <div className="flex items-center justify-end gap-2" onClick={ev => ev.stopPropagation()}>
          <button onClick={() => openEdit(e)} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600">
            <Pencil size={14} />
          </button>
          <button onClick={() => remove(e.id)} className="p-1.5 rounded-lg hover:bg-red-50 text-gray-400 hover:text-red-500">
            <Trash2 size={14} />
          </button>
        </div>
      ),
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <p className="text-lg font-bold text-gray-900">{formatCurrency(totalBase)}</p>
        </div>
        <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <p className="text-lg font-bold text-red-600">{formatCurrency(totalAll)}</p>
        </div>
      </div>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="Ej: Alquiler equipo de cámara"
            />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Importe (sin IVA) *</label>
            <input
              type="number"
              min="0"
              step="0.01"
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={save} className="px-4 py-2 text-sm bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium">
            {selected ? 'Guardar cambios' : 'Registrar gasto'}
          </button>
        </div>
      </Modal>
    </div>
